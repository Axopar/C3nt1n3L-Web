...
function generarRespuesta(pregunta) {
  ...
  // Validación de datos
  if (!pregunta) return "Por favor, ingresa una pregunta válida.";
  ...
  if (pregunta.toLowerCase().includes("contrato")) {
    ...
    respuesta = conocimientoJuridico.general.respuesta || "No tengo una respuesta para eso.";
  }

  return respuesta;
}

// Estructura de conocimiento jurídico
const conocimientoJuridico = {
  derechoCivil: {
    contratos: {
      respuesta: "Esta es una respuesta sobre contratos"
    },
    general: {
      respuesta: "Información general sobre derecho civil"
    }
  },
  general: {
    respuesta: "Información general sobre cuestiones legales"
  }
};
...
function enviarPregunta() {
  ...
  if (!elementoPregunta || !elementoRespuesta) {
    console.error("Elementos del DOM no encontrados.");
    return;
  }
  
  const pregunta = elementoPregunta.value;
